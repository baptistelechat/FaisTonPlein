...
function RouteLayer({ route }: { route: RouteGeometry | null }) {
  ...
  useEffect(() => {
    ...
    map.addLayer({
      ...
      paint: { 'line-color': ROUTE_COLOR, 'line-width': 2, 'line-opacity': 0, 'line-dasharray': [6, 4] },
    })
  ...
  useEffect(() => {
    ...
    if (!route) {
      ...
      if (map.getLayer(ROUTE_ROAD_LAYER)) map.setPaintProperty(ROUTE_ROAD_LAYER, 'line-opacity', 0)
      if (map.getLayer(ROUTE_CROW_LAYER)) map.setPaintProperty(ROUTE_CROW_LAYER, 'line-opacity', 0)
      return
    }
    ...
    if (!route.isRoad) {
      ...
      const fadeIn = (timestamp: number) => {
        ...
        if (map.getLayer(ROUTE_CROW_LAYER)) {
          map.setPaintProperty(ROUTE_CROW_LAYER, 'line-opacity', easeOutCubic(progress) * 0.65)
        }
    ...
    const animate = (timestamp: number) => {
      if (animStartRef.current === null) animStartRef.current = timestamp
      const progress = Math.min((timestamp - animStartRef.current) / ROAD_ANIMATION_MS, 1)
      ...
      source.setData({
        type: 'Feature',
        properties: {},
        geometry: { type: 'LineString', coordinates: allCoords.slice(0, numPoints) },
      })
      if (map.getLayer(ROUTE_ROAD_LAYER)) {
        map.setPaintProperty(ROUTE_ROAD_LAYER, 'line-opacity', Math.min(eased * 1.5, 0.85))
      }
