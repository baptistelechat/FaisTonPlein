...
const defaultStyles = {
  ...
  // Using OSM Bright style (or similar open style) which usually defaults to local names or supports language fields
  // Carto Positron is good but English-heavy. Let's try to inject language if possible or switch to a style that respects it.
  // For now, keeping Carto but we might need a custom style URL for forced French.
  // Alternative: "https://tile.openstreetmap.org/{z}/{x}/{y}.png" (raster) but we need vector style.
  // Let's use a style that supports French if available, or stick to standard and rely on browser language.
  // Actually, MapTiler or others allow language setting.
  // For this "zero config" map, we are limited to public styles.
  // Let's try to use a style that might be more locale-friendly or just accept Carto's default.
...
const Map = forwardRef<MapRef, MapProps>(function Map(
  ...
  useEffect(() => {
    ...
    const handleStyleData = () => {
      ...
      // Attempt to force French labels
      try {
          ...
          style.layers.forEach((layer) => {
            if (layer.id.includes("label") || layer.type === "symbol") {
              ...
              if (layout && layout["text-field"]) {
                // Check if we can use name:fr
                // This is heuristic and depends on the vector tile source schema
                // Ideally we should check if name:fr exists in source but that's per-tile
                // We set a coalesce expression: try name:fr, then name_fr, then name
